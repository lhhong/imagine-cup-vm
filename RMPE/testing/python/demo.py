...
def getposture(imagelink, param, model, x_coord, y_coord):
    ...
    path = imagelink.split('.')[0]

    plt.savefig(path+'-foo.jpg')

    plt.imshow(oriImg[:,:,[2,1,0]])
    fig = plt.imshow(oriImg[:,:,[2,1,0]])
    ...
    ### FINDING FACE THAT MATCHES COORDS
    distances = []

    for person in subset:
        nose_x = candidate[int(person[0])][0]
        nose_y = candidate[int(person[0])][1]
        dist = (x_coord - nose_x)**2 + (y_coord - nose_y)**2
        distances.append(dist)

    if len(distances) > 0:
        x = distances.index(min(distances))
    else:
        print ('No face found!')
        return
    #x is the index of the person whose face we are looking for

    try:
        for i in range(18):
            if subset[x][i] > 0:
                body_x[i] = candidate[int(subset[x][i])][0]
                body_y[i] = candidate[int(subset[x][i])][1]
    except IndexError:
        print ('NO FACE FOUND')
        return

    try:
        top_max_x = max(body_x.get(1,None), body_x.get(2,None), body_x.get(5,None), body_x.get(3,None), body_x.get(4,None), body_x.get(6,None), body_x.get(7,None), body_x.get(8,None), body_x.get(11,None))
        top_min_x = min(a for a in (body_x.get(1,None), body_x.get(2,None), body_x.get(5,None), body_x.get(3,None), body_x.get(4,None), body_x.get(6,None), body_x.get(7,None), body_x.get(8,None), body_x.get(11,None)) if a is not None)
    ...
    try:
        bot_max_x = max(body_x.get(8,None), body_x.get(11,None), body_x.get(9,None), body_x.get(10,None), body_x.get(12,None), body_x.get(13,None))
        bot_min_x = min(a for a in (body_x.get(8,None), body_x.get(11,None), body_x.get(9,None), body_x.get(10,None), body_x.get(12,None), body_x.get(13,None)) if a is not None)
    ...
    #get position 1/5 extrapolated from knee to ankle. Expand box around point by 1/2 the knee-ankle length all 4 sides. 
    points = [body_x.get(9, None), body_x.get(10, None), body_y.get(9, None), body_y.get(10, None)]
    applicable = True
    for point in points:
        if (point is None):
            applicable = False
    ...
    if (applicable):
        shinlength = np.sqrt(np.square(points[0]-points[1]) + np.square(points[2]-points[3]))
        shoe_x = points[1]+(points[1]-points[0])/5
        shoe_y = points[3]+(points[3]-points[2])/5
        extend = int(shinlength/3)
        extend1 = int(shinlength/2.5)
        plt.xlim([shoe_x-extend1, shoe_x+extend])
        plt.ylim([shoe_y+extend, shoe_y-extend])
...
if __name__ == '__main__':
    print ('call the function in this script')
